...
export const generateQRCodeForUser = async (req, res) => {
  try {
    ...
    let qrRecord = await QRCodeModel.findOne({ unique_code });
    if (!qrRecord) {
...
export const scanZone = async (req, res) => {
  try {
    ...
    const client = await Client.findOne({ unique_code });
    if (!client) return res.status(404).json({ message: "Client not found" });

    // Get the last scan for this client
    const lastScan = await ZoneTracker.findOne({ client_id: client._id })
      .sort({ scanned_at: -1 });

    // ✅ If zone_id is provided → Enter/Move
    if (zone_id) {
      // If client was in a different zone previously, decrement that zone's count
      if (lastScan && lastScan.current_zone_id?.toString() !== zone_id) {
        if (lastScan.current_zone_id) {
          await Zone.findByIdAndUpdate(lastScan.current_zone_id, {
            $inc: { client_count: -1 },
          });
        }
      }
      ...
      await Zone.findByIdAndUpdate(zone_id, {
        $inc: { client_count: 1 },
      });
...
export const getZoneDensity = async (req, res) => {
  try {
    const zones = await Zone.find({}, "name client_count");
...
export const getUserZoneHistory = async (req, res) => {
  try {
    ...
    // Fetch scan history
    const scans = await ZoneTracker.find({ client_id })
      .populate("current_zone_id", "name")
      .populate("last_zone_id", "name")
      .sort({ scanned_at: 1 });
    ...
    const history = scans.map((scan, i) => {
      ...
      return {
        last_zone: scan.last_zone_id ? scan.last_zone_id.name : null,
        current_zone: scan.current_zone_id ? scan.current_zone_id.name : null,
