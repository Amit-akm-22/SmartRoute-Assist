...
const AdminPage = () => {
    ...
    const ADMIN_API_URL = `${BACKEND_URL}/admin`;
    ...
    const fetchDashboardData = async () => {
        ...
        try {
            // Fetch Stats
            const statsRes = await fetch(`${ADMIN_API_URL}/stats`, { headers });
            ...
            // Fetch Users
            const usersRes = await fetch(`${ADMIN_API_URL}/users`, { headers });
            setUsers(await usersRes.json());

            // Fetch Lost Items
            const lostRes = await fetch(`${ADMIN_API_URL}/lostfound`, { headers });
            setLostItems(await lostRes.json());

            // Fetch Tickets/Bookings
            const ticketsRes = await fetch(`${ADMIN_API_URL}/tickets`, { headers });
            setBookings(await ticketsRes.json());

            // Fetch Density
            const densityRes = await fetch(`${ADMIN_API_URL}/density`, { headers });
