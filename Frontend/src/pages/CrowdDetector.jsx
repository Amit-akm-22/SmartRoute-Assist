...
const CrowdDetector = () => {
    ...
    const CROWD_API_URL = `${BACKEND_URL}/crowd`;

    // Fetch backend status
    const updateStatus = async () => {
        try {
            const res = await fetch(`${CROWD_API_URL}/api/status`);
    ...
    const fetchZoneData = async () => {
        ...
        try {
            const url = mode === 'camera'
                ? `${CROWD_API_URL}/api/zones`
                : `${CROWD_API_URL}/api/upload_zones/${sessionToken}`;
    ...
    const handleStartCamera = async () => {
        setIsStarting(true);
        try {
            const res = await fetch(`${CROWD_API_URL}/api/start`);
            const data = await res.json();
            if (res.ok) {
                setIsStreaming(true);
    ...
    const handleStopCamera = async () => {
        try {
            const res = await fetch(`${CROWD_API_URL}/api/stop`);
            if (res.ok) {
    ...
    const handleFileUpload = async (e) => {
        ...
        try {
            const res = await fetch(`${CROWD_API_URL}/api/upload-video`, {
                method: 'POST',
                body: formData
            });
            const data = await res.json();
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                    ...
                    <div className="lg:col-span-2 space-y-4 md:space-y-6">
                        <div className="relative aspect-[4/5] md:aspect-video bg-black rounded-3xl md:rounded-[2rem] overflow-hidden border border-gray-200 shadow-2xl group">
                            {isStreaming ? (
                                <img
                                    src={mode === 'camera' ? `${CROWD_API_URL}/video_feed` : `${CROWD_API_URL}/uploaded_feed/${sessionToken}`}
                                    alt="Live Stream"
