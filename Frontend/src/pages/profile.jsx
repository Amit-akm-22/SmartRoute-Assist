...
console.log("📍 [Profile] Using Backend URL:", BACKEND_URL);
...
const ProfileRfidPage = () => {
  ...
  const generateRfid = async (profileUser) => {
    ...
    try {
      const token = localStorage.getItem("token");
      const res = await fetch(`${BACKEND_URL}/zone/generate`, {
        method: "GET",
        headers: { Authorization: `Bearer ${token}` },
      });
      const data = await res.json();
                  ...
                  <div className="relative group/avatar cursor-pointer">
                    <div className="w-32 h-32 sm:w-36 sm:h-36 rounded-full p-1 bg-white border-2 border-slate-100 shadow-sm transition-transform active:scale-95">
                      <div className="w-full h-full rounded-full bg-slate-50 overflow-hidden flex items-center justify-center">
                        {previewUrl || (user.profile_image ? (user.profile_image.startsWith("http") ? user.profile_image : `${BACKEND_URL.replace('/api/v1', '')}${user.profile_image}`) : null) ? (
                          <img
                            src={previewUrl || (user.profile_image.startsWith("http") ? user.profile_image : `${BACKEND_URL.replace('/api/v1', '')}${user.profile_image}`)}
                            alt="Profile"
